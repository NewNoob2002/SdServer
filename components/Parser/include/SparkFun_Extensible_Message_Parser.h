...
extern uint8_t bluetoothDataBufer[256];
//----------------------------------------
// Types
//----------------------------------------
...
typedef struct _SEMP_Bluetooth_VALUES
{
    uint32_t crc;            // Copy of CRC calculation before CRC bytes
    uint16_t bytesRemaining; // Bytes remaining in RTCM CRC calculation
} SEMP_Bluetooth_VALUES;
...
typedef union
{
    ...
    SEMP_Bluetooth_VALUES bluetooth;
} SEMP_SCRATCH_PAD;
...
typedef struct _SEMP_Bluetooth_HEADER
{
    uint8_t syncA;          // 0xaa
    uint8_t syncB;          // 0x44
    uint8_t syncC;          // 0x18
